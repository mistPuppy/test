        ...
        <div class="pre">
            ...
            <div class="pre" id="table-responsive">
                <table style="text-align: center;" class="table table-bordered table-striped table-hover">
                    ...
                    <tbody style="color: #858796;">
                        <?php
                        $title_name = $_GET['title_name'];
                        $title_num = $_GET['title_num'];
                        // echo $title_name, $title_num;
                        $link = mysqli_connect("localhost", "root", "12345678", "finalwork");
                        if (empty($searchtxt)) {
                            $sql = "select * from finalworkteam where team_專題編號 like '%$title_name%' && team_專題名稱 like '%$title_num%'";
                            // echo $sql;
                        } else {
                        ...
                        while ($record = mysqli_fetch_row($rs)) {
                            ...
                            <td>$record[7]</td>
                            <td>$record[8]</td>
                            <td style='width:60px'><button class='form-control' style='width:60px; background-color: #f6c23e'><a style='color:white; width:60px' href=修改專題成員.php?team_id=$record[0]>修改</button></a></td>
                            <td style='width:60px'><button class='form-control' style='width:60px; background-color: #e74a3b'><a onclick='myFunction()' style='color:white;width:60px' href=delete.php?method=delete&team_id=$record[0]>刪除</a></button></td>
                            </tr>
                            ";
                        }
